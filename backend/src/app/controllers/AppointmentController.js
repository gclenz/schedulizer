...
class AppointmentController {
  ...
  async store(req, res) {
     ...
     */
    const checkIfIsProvider = await User.findOne({
      where: { id: provider_id, provider: true },
    });

    if (!checkIfIsProvider) {
      return res
        .status(401)
    ...
    if (checkAvailability) {
      return res
        .status(400)
        .json({ error: 'Appointment date/hour not available.' });
    }
  ...
  async delete(req, res) {
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res.status(401).json({
        error:
          'You can only cancel appointments with maximum of 2 hours before it.',
      });
    }
